...
public static class IntegrityAudit
{
    ...
    public sealed record AgentChain(
        string AgentId, string SeatId, int Total, int HashOk, int ChainOk,
    ...
    public sealed record Report(
        string ExamId, int TotalEvents, int TotalHashOk, int TotalChainOk, IReadOnlyList<AgentChain> Agents)
    {
    ...
    public static Report Run(SqliteConnection conn, string examId)
    {
        ...
        int totalEvents = 0, totalHashOk = 0, totalChainOk = 0;
        ...
        foreach ((string agentId, List<Row> rows) in byAgent)
        {
            ...
            agents.Add(new AgentChain(
                agentId, seatOf[agentId], rows.Count, rows.Count - hashMiss.Count, rows.Count - chainMiss.Count,
                hashMiss, chainMiss));
        }

        return new Report(examId, totalEvents, totalHashOk, totalChainOk, agents);
    }
