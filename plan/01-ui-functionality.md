...
## 待确认（需你/owner 拍板）
> **已于 2026-07-15 全部确认并落地，本条关闭。**
- U3：**已选方案 A**（健康信号继续留在 `suspicious_queue`，加 `source` 列区分；看板独立「采集健康」面板呈现）。见 U3 / 03-D2 / 05-M8。
- U1：**确认暴露**「点名抓图」按钮到看板（非「按预期行为」）。见 U1 / 05-M1 / 06-broken X1。
