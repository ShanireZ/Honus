...
- **M4 健壮性 / 身份层**：**✅ cpplearn OIDC 取代共享 PSK**（采集面 per-user 身份闭合 §10.1 事件通道栽赃 + seq 抢占，拓扑 A Server-Broker）+ **✅ RBAC 角色映射**（弟子=考生 / 长老=监考员·OIDC 取代看板静态令牌·远端工作站经自签 https·详见 [m4-identity-oidc.md](m4-identity-oidc.md) §10）+ **✅ 考前预检**（`/api/preflight`：鉴权配置 / cpplearn 可达 / active 考试白名单覆盖）。**⏳ 待做**：Agent 防卸载 / 保活、多服务器水平扩展、both→oidc 现场灰度验收。
