...
| D4 | 图像分析位置 | 服务器集中 + **视觉 LLM(识图)** —— 取代专用云 OCR + L3 Logo,合并单一视觉级(2026-07-01 owner 拍板,详见 §4)。provider-agnostic;**已选定 = 小米 MiMo-V2.5 托管 API**(`token-plan-cn.xiaomimimo.com/v1`·OpenAI 兼容·**境内云**·PIPL 无跨境);key DPAPI 加密存储不存明文 |
