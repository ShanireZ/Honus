...
- **管理面 HMAC 令牌**（`X-Horus-Admin`）：`/api/*` 看板与管理写端点独立鉴权，防同网学员机操纵 config / 裁决 / 拉证据图。
- 可靠性收口（M1 已实现）：事件先落盘再发、**逐条 ack 精确删除**（非范围 upto，避免共享 seq 空洞误删证据）、**序号高水位持久化**（重启不复用 seq）、断线指数退避重连 + 续传、缓冲原子压实 + 崩溃恢复。

### 10.1 M1 已知残留风险（诚实标注，属威胁模型本质，非缺陷）
- **PSK 驻留学员机**：Agent 跑在学员机、PSK 在本地配置。持 PSK 者可对事件正确签名，而 **M1 服务器只验 `sig=HMAC(PSK, hashSelf+seq)`、不重算 canonical、不验哈希链连续性、全信 Agent 的 `risk`** → 学员理论上可签一条 `risk=0` 的"访问 AI 站"事件逃逸可疑队列。**完整 canonical 复算 + 哈希链复验属 M3**；M1 靠"系统只初筛、人工裁决 + 物理监考兜底"哲学承接。缓解：每 Agent 独立 PSK（泄露隔离到单座位）、密钥用机器保护而非明文。
- **签名无时效防重放**：M1 `sig` 不含服务器时钟窗口；捕获的合法签名帧可重放（事件靠 `(agent,seq)` 去重、图片靠 imageId 去重，keystroke 无去重）。M2 可把 `ts` 纳入签名并校验窗口。
- **击键旁路无签名**：`/ingest/keystroke` 来自判题前端、无法安全持 PSK，故无签名、`seatId` 自述可被伪造/栽赃。应绑判题前端会话令牌或限来源 IP（M2）。
- **PSK / 管理令牌留空 = 放行**：仅供本机联调。**已 fail-closed**：非 loopback 绑定却缺 PSK 或管理令牌时**拒绝启动**（`allowInsecure=true` 仅联调可绕）。
...
- **M2 结构性待办**：服务器 **单连接 + 全局写锁**是吞吐天花板 → 看板只读查询应走独立只读连接（WAL 并发读）；信号入队走单写线程/Channel 而非热路径同步写盘；**归档/清理定时任务尚未实现**（schema 承诺 30 天转 archive，当前无执行者 → 长期常驻会无界增长，属 §13/§15 M3）；心跳在哈希链上，归档 DELETE 会断链（M3 哈希链复验时需分段容忍）。
