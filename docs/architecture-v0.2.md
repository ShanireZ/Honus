...
- 状态：**M1–M5 + 考试派发均已实现·221 测试全绿**（Windows 专属采集端硬化待 owner 真机验收，见 §15）
...
- **M5 采集端硬化**：**✅ 已实现**——保活 / 防挂起 / 防遮蔽 / 防降权限统一为"检测 + 上报 + 看板健康告警"(4 新健康信号 + 服务器独立赋分入队 + 座位健康度)+ **三层保活看门狗**(Windows 服务 LocalSystem / 兄弟 watchdog 互拉 / 心跳告警)。纯检测·**不做内核对抗**(owner 拍板)。平台无关检测核 + 服务端 176 测试全绿(M5 批次·累计 **221 全绿**);Windows 专属(服务 / SessionLauncher / 真实 suspend·遮蔽·降权)待 owner 真机验收。详见 [m5-agent-hardening.md](m5-agent-hardening.md)。
