...
- **✅ M3 已实现（2026-07-03·owner 拍板）**：**provider-agnostic 图像嵌入器**（`IImageEmbedder`）；**规模小(单场几千图)→ C# 暴力余弦 KNN,不依赖 sqlite-vec 原生扩展**（`vec_images` 虚表留大规模余量,embedding 存普通 `image_embeddings` BLOB 表）；**仅嵌证据图**(is_evidence·省算力·后台 `ImageEmbedService` 补扫)；`POST /api/exams/{id}/search-image` 暴力余弦 top-N + 看板灯箱「按图搜图」。
  ...
  - **部署**:`embedProvider=onnx` + `embedOnnxModelPath`(CLIP ViT-B/32 图像编码器 ONNX·可从 HuggingFace `Xenova/clip-vit-base-patch32` 取 vision_model.onnx);启动日志「ONNX CLIP 嵌入器就绪」即成。
