...
- **✅ M3 已实现（2026-07-03·owner 拍板）**：**provider-agnostic 图像嵌入器**（`IImageEmbedder` + `MockImageEmbedder` 测试 + `OpenAiImageEmbedder` OpenAI 兼容 `/v1/embeddings`·**默认复用视觉 baseUrl/key**·送派生图·原图不出网）；**规模小(单场几千图)→ C# 暴力余弦 KNN,不依赖 sqlite-vec 原生扩展**（`vec_images` 虚表留作大规模余量,embedding 存普通 `image_embeddings` BLOB 表）；**仅嵌证据/可疑图**(is_evidence·省算力·后台 `ImageEmbedService` 补扫)；`POST /api/exams/{id}/search-image` 暴力余弦 top-N + 看板灯箱「按图搜图」。⚠️ 真嵌入端点须有图像/多模态 embedding 模型(纯文本端点不适用→可切本地 ONNX·同一接口)。
