...
| D4 | 图像分析位置 | 服务器集中 + **视觉 LLM(识图)** —— 取代专用云 OCR + L3 Logo,合并单一视觉级(2026-07-01 owner 拍板,详见 §4)。provider-agnostic:OpenAI 兼容端点(DeepSeek-V4 / 小米 MiMo-V2.5 / Qwen-VL / GLM-4V);MiMo 开源可**本地自托管 → 零出网** |
...
L2 现为**视觉 LLM 识图**(取代 OCR,见 §4)。走**云端点**时这是全系统唯一离开局域网的环节,按下列约束收口;走**本地自托管**(开源 MiMo + vLLM)时则**零出网**、收口第 4/5 条不适用。**注意**:当前实现的裁剪(第 2 条)/打码身份(第 3 条)为**直传 stub**,接真云端点前必须补齐(见 §15 待收尾)。
...
- **M2 分析增强**：**L2 视觉 LLM(识图·取代 OCR+Logo)** + 风险评分 + 可疑队列。**✅ 骨架已实现**(provider-agnostic `IVisionAnalyzer` + Mock + OpenAI 兼容 adapter + 异步后台分析 → `ocr_results` + `server_risk` 聚合 + 可疑队列;`visionProvider` config 换供应商)。**待收尾**:接真端点(owner 定 DeepSeek 云 vs 自托管 MiMo)+ §5 真裁剪/打码身份(现直传 stub)+ 基线抽样策略。
