...
- 日期：2026-07-02（RBAC 增补 + §11 灰度验收清单 2026-07-03）· 状态：**采集面 OIDC + RBAC(S8/S9) + both→oidc 灰度验收清单/工具均已实现·180 测试全绿（M4 里程碑时·累计 225 全绿）·真机 smoke 通过**（cpplearn 两批改动待 owner commit）
...
**✅ 单测**：cpplearn 全量 `test:server` 全绿（**75 unit + 65 integration = 585 测试**·外部仓）+ Horus **142 测试**（S1–S7 阶段·含 A1/A2 端到端闭合·累计 **225 全绿**）。
