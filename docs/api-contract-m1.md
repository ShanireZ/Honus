...
>
> **M3 增补(管理面,已实现)**：
> - `GET /api/exams/{examId}/integrity` — 哈希链完整性离线审计。返回 `{ ok, totalEvents, totalHashOk, totalChainOk, totalUnverifiable, agents:[{agentId,seatId,total,hashOk,chainOk,unverifiable,hashMismatches[],continuityBreaks[]}] }`；已归档考试返回 `{ status:"archived", applicable:false }`，不存在返回 404。`unverifiable` = 迁移前缺 `machine_id` 的旧事件(不可复算,**非篡改**)。
