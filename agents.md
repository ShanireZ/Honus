...
- ✅ **§5 隐私收口已实现**(`server/Analysis/Vision/VisionImagePrep.cs`·ImageSharp 3.1.12):送云前**打码身份**(`visionRedactRects` 遮姓名/学号)+ **裁剪**(`visionCropRect` 只留答题区)+ **降采样**(`visionMaxEdge` 默认 1600)再重编码 WebP;**原图字节只读、永不出网**,只送派生图;要打码却解码失败则跳过分析而非泄漏;无收口配置则直通不解码(mock 测试走此路)。矩形用归一化坐标(分辨率无关)。
- ⏳ **待收尾**:① 按各考场 UI 布局配 `visionRedactRects`(姓名/学号在屏何处·部署项);② 基线抽样策略。再往后 M3（完整 canonical 复算 + 哈希链复验 / 归档清理任务 / CLIP 按图搜图）。里程碑见 architecture §15。
